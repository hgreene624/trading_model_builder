...
- **Holdout blending:** when train/test metrics are available, the EA scores each candidate via `score = w_test * test_score + w_train * bonus(train_score) - gap_penalty`, where `w_test` defaults to 0.65, the bonus only credits the training window when it is within `holdout_gap_tolerance` of the test score, and the penalty activates if training outperforms the holdout. The applied components are logged per individual for inspector tooling.【F:src/optimization/evolutionary.py†L1002-L1061】
